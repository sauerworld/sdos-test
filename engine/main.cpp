...
void setupscreen(int &useddepthbits, int &usedfsaa)
{
    ...
    int inix = SDL_WINDOWPOS_UNDEFINED, iniy = SDL_WINDOWPOS_UNDEFINED, iniw = scr_w, inih = scr_h, flags = SDL_WINDOW_RESIZABLE;
    ...
    loopi(sizeof(configs)/sizeof(configs[0]))
    {
        ...
        screen = SDL_CreateWindow("Cube 2: Sauerbraten", inix, iniy, iniw, inih, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | flags);
        if(screen) break;
    }
...
static bool filterevent(const SDL_Event &event)
{
    switch(event.type)
    {
        case SDL_MOUSEMOTION:
            ...
            {
                if(event.motion.x == winw / 2 && event.motion.y == winh / 2) 
...
static void resetmousemotion()
{
    if(grabinput && !relativemouse && !(SDL_GetWindowFlags(screen) & SDL_WINDOW_FULLSCREEN))
    {
        SDL_WarpMouseInWindow(screen, winw / 2, winh / 2);
    }
}
...
int main(int argc, char **argv)
{
    ...
    conoutf(stringify_macro(\f0Sauerbraten \f2C\f6o\f3m\f5m\f1u\f1n\f0i\f2t\f6y \f3E\f5d\f1i\f0t\f2i\f6o\f3n\f0: nightly build:\f3)" %s",__DATE__); //Release \f3v0.1));
