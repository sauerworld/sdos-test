...
#endif
{
    ...
    void drawblip(fpsent *d, float x, float y, float s, int i, bool flagblip)
    {
        ...
        settexture(m_hold && (!flagblip || !f.owner || lastmillis%1000 < 500) ? (flagblip ? "packages/hud/blip_neutral_flag.png" : "packages/hud/blip_neutral.png") :
                    ((m_hold ? ctfteamflag(f.owner->team) : f.team)==ctfteamflag(autohudplayer()->team) ?
    ...
    void rendergame()
    {
        loopv(flags)
        {
            ...
            const char *flagname = m_hold && (!f.owner || lastmillis%1000 < 500) ? "flags/neutral" : (m_hold ? ctfteamflag(f.owner->team) : f.team)==ctfteamflag(autohudplayer()->team) ? "flags/blue" : "flags/red";
            float angle;
            ...
            {
                particle_flare(pos, f.spawnloc, 0, PART_LIGHTNING, strcmp(f.owner->team, autohudplayer()->team) ? 0xFF2222 : 0x2222FF, 1.0f);
                if(!flags.inrange(f.owner->lastbase))
                {
                    particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(f.owner->team, autohudplayer()->team) ? 0x802020 : 0x2020FF, 4.8f);
                    particle_splash(PART_SPARK, 50, 250, pos, strcmp(f.owner->team, autohudplayer()->team) ? 0x802020 : 0x2020FF, 0.24f);
                }
                f.owner->lastbase = i;
            }
        }
    ...
    void flagexplosion(int i, int team, const vec &loc)
    {
        ...
        else if(team==ctfteamflag(autohudplayer()->team)) { fcolor = 0x2020FF; color = vec(0.25f, 0.25f, 1); }
    ...
    void flageffect(int i, int team, const vec &from, const vec &to)
    {
        ...
        if(from.x >= 0 && to.x >= 0)
            particle_flare(fromexp, toexp, 600, PART_LIGHTNING, !team ? 0xFFC0A0 : (team==ctfteamflag(autohudplayer()->team) ? 0x2222FF : 0xFF2222), 1.0f);
    }
    ...
    void scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team, int score, int dflags)
    {
        ...
        playsound(team==ctfteamflag(autohudplayer()->team) ? S_FLAGSCORE : S_FLAGFAIL);
