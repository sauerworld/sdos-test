...
#endif
{
    ...
    void rendertether(fpsent *d)
    {
        ...
        if(d->state==CS_ALIVE)
        {
            loopv(bases)
            {
                ...
                particle_flare(pos, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - AMMOHEIGHT - 4.4f), 0, PART_LIGHTNING, strcmp(d->team, autohudplayer()->team) ? 0xFF2222 : 0x2222FF, 1.0f);
                if(oldbase < 0)
                {
                    particle_fireball(pos, 4.8f, PART_EXPLOSION, 250, strcmp(d->team, autohudplayer()->team) ? 0x802020 : 0x2020FF, 4.8f);
                    particle_splash(PART_SPARK, 50, 250, pos, strcmp(d->team, autohudplayer()->team) ? 0x802020 : 0x2020FF, 0.24f);
                }
                d->lastbase = i;
            }
        }
    ...
    void rendergame()
    {
        ...
        loopv(bases)
        {
            baseinfo &b = bases[i];
            if(!b.valid()) continue;
            const char *basename = b.owner[0] ? (strcmp(b.owner, autohudplayer()->team) ? "base/red" : "base/blue") : "base/neutral";
            ...
            regular_particle_flame(PART_FLAME, vec(b.ammopos.x, b.ammopos.y, b.ammopos.z - 4.5f), fradius, fheight, b.owner[0] ? (strcmp(b.owner, autohudplayer()->team) ? 0x802020 : 0x2020FF) : 0x208020, 3, 2.0f);
...
//            particle_fireball(b.ammopos, 4.8f, PART_EXPLOSION, 0, b.owner[0] ? (strcmp(b.owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 4.8f);
            ...
            if(b.owner[0])
            {
                bool isowner = !strcmp(b.owner, autohudplayer()->team);
            ...
            else if(b.enemy[0])
            {
                ...
                if(strcmp(b.enemy, autohudplayer()->team)) { tcolor = 0xFF4B19; mtype = PART_METER; mcolor = 0xFF1932; }
    ...
    void updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
    {
        ...
        if(owner[0])
        {
            if(strcmp(b.owner, owner))
            {
                ...
                if(!strcmp(owner, autohudplayer()->team)) playsound(S_V_BASECAP);
            }
        }
        else if(b.owner[0])
        {
            ...
            if(!strcmp(b.owner, autohudplayer()->team)) playsound(S_V_BASELOST);
        }
        if(strcmp(b.owner, owner)) particle_splash(PART_SPARK, 200, 250, b.ammopos, owner[0] ? (strcmp(owner, autohudplayer()->team) ? 0x802020 : 0x2020FF) : 0x208020, 0.24f);
        copystring(b.owner, owner);
        copystring(b.enemy, enemy);
        b.converted = converted;
        if(ammo>b.ammo)
    ...
    void setscore(int base, const char *team, int total)
    {
        ...
        else if(bases.inrange(base))
        {
            ...
            if(!strcmp(b.owner, team))
            {
                ...
                particle_textcopy(above, msg, PART_TEXT, 2000, isteam(team, autohudplayer()->team) ? 0x6496FF : 0xFF4B19, 4.0f, -8);
            }
        }
    }
