...
#endif
{
    ...
    {
        base &b = bases[i];
        settexture(b.team==collectteambase(autohudplayer()->team) ? "packages/hud/blip_blue.png" : "packages/hud/blip_red.png", 3);
    ...
    {   
        ...
        settexture(team == collectteambase(autohudplayer()->team) ? "packages/hud/blip_red_skull.png" : "packages/hud/blip_blue_skull.png", 3);
        loopv(players)
        {
            fpsent *o = players[i];
    ...
    void rendergame()
    {
        int team = collectteambase(autohudplayer()->team);
        vec theight(0, 0, 0);
    ...
    void baseexplosion(int i, int team, const vec &loc)
    {
        int fcolor;
        vec color;
        if(team==collectteambase(autohudplayer()->team)) { fcolor = 0x2020FF; color = vec(0.25f, 0.25f, 1); }
    ...
    void baseeffect(int i, int team, const vec &from, const vec &to, bool showfrom = true, bool showto = true)
    {
        ...
        particle_flare(from, to, 600, PART_LIGHTNING, team==collectteambase(autohudplayer()->team) ? 0x2222FF : 0xFF2222, 1.0f);
    }
    ...
    token *droptoken(fpsent *d, int id, const vec &o, int team, int yaw, int n)
    {
        ...
        token &t = droptoken(id, pos, yaw, team, lastmillis);
        ...
        if(d) 
        {
            if(!n)
            {
                particle_fireball(d->o, 4.8f, PART_EXPLOSION, 500, team==collectteambase(autohudplayer()->team) ? 0x2020FF : 0x802020, 4.8f);
                particle_splash(PART_SPARK, 50, 250, d->o, team==collectteambase(autohudplayer()->team) ? 0x2020FF : 0x802020, 0.24f);
            }
            particle_flare(d->o, vec(t.o.x, t.o.y, t.o.z + 0.5f*(TOKENHEIGHT + 1)), 500, PART_LIGHTNING, team==collectteambase(autohudplayer()->team) ? 0x2222FF : 0xFF2222, 1.0f); 
        }
        return &t;
    }
    ...
    void stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam, int score)
    {
        ...
        if(bases.inrange(basenum))
        {
            ...
            if(t) particle_flare(b.tokenpos, vec(t->o.x, t->o.y, t->o.z + 0.5f*(TOKENHEIGHT + 1)), 500, PART_LIGHTNING, team==collectteambase(autohudplayer()->team) ? 0x2222FF : 0xFF2222, 1.0f);
        }
    }
    ...
    void deposittokens(fpsent *d, int basenum, int deposited, int team, int score, int flags)
    {
        ...
        playsound(team==collectteambase(autohudplayer()->team) ? S_FLAGSCORE : S_FLAGFAIL);
